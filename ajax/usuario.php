...
$id_microred = isset($_POST['microRed']) ? limpiarCadena($_POST['microRed']):"";

$redMicro = isset($_GET['redMicro']) ? limpiarCadena($_GET['redMicro']):"";
...
switch ($_GET['op']) {
	...
	if (empty($id_usuario)){
		...
		$rspta = $datos->insertarUsuario($tipodocidentidad, $numdoc_usuario, strtoupper($apenom_usuario), $numtele_usuario, $email_usuario, $tipousuario, $red, $usuario_sistema, $password_sistema,$id_microred);

		echo $rspta ? "Usuario al sistema registrado correctamente." : "Usuario al sistema no se pudo registrar";

	}else {

		$rspta = $datos->editarUsuario($id_usuario, $tipodocidentidad, $numdoc_usuario, strtoupper($apenom_usuario), $numtele_usuario, $email_usuario, $tipousuario, $red, $usuario_sistema, $password_sistema,$id_microred);
	...
	<option value = "COLLAO">COLLAO</option>
	...
	/*========================================================================
	// caso para obtener una microred en base a la red
	========================================================================*/
	
	case 'selectMicrored':

		$rspta = $datos->listarMicroredsegunRedes($redMicro);

		echo '<option value=0>SELECCIONE</option>';

		while ($reg = $rspta->fetch_object()) {

			echo '<option value="' . $reg->id_microred . '">' . $reg->descripcion_microred . '</option>';
		}

		break;

	/*========================================================================
	// fin del caso para obtener una microred en base a la red
	========================================================================*/
	...
	if (isset($fetch)) {
		...
		$_SESSION['id_microred'] = $fetch->id_microred;
